...
import aws.cfn.dlmodel.{Model, ModelIRI, ModelType => Type}
...
private class ResourceSpecificationModelMapper(resSpec : ResourceSpecification)
{
  ...
  private def owlPropertyFromGenericPropertyType(propertyType: GenericPropertyType) =
    propertyType match {
      ...
      case PolicyProperty(n,d,r)                  => axioms(n,d.name, Policy.PolicyDocument,r,f = true)
  ...
  private def axs(p: String, d: String, ra: OWL2Datatype,
    ...
    addPropertyToOntology(p, isObjProp)
      ...
      domainAxiom(p, d) ++
      rangeAxiom(p, ra) ++
      requiredAxiom(d, p, r, ra) ++
      functionalAxiom(d, p, f)
  }
  ...
  private def owlClass(resName: String, otherResSpec: String = m.name) = {
    if (resName==null)
      None
    else if (resName.equals(Policy.PolicyDocument))
      Some(m.df
        .getOWLClass(ModelIRI
          .resourceTypeIRI(
            Policy.PolicyDocument, resName)))
    else
      Some(m.df
        .getOWLClass(ModelIRI
          .resourceTypeIRI(
