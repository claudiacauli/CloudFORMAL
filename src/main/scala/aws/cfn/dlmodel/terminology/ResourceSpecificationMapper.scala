...
class ResourceSpecificationMapper(resSpec : ResourceSpecification) {
  ...
  def map(resSpec : ResourceSpecification) : ResourceSpecificationModel = {
    val m : ResourceSpecificationModel = new ResourceSpecificationModel(resSpec.name)
    val im = new InnerMapper(m)
    m.ontology.add((im.owlClassFromResourceType(resSpec.resType) ++ (resSpec.subPropTypes flatMap im.owlClassFromSubpropertyType)).asJava)
    m
  }
  ...
  class InnerMapper(val m : ResourceSpecificationModel){

    def owlClassFromResourceType(resourceType: ResourceValueType): Vector[OWLAxiom] = {
    ...
    def owlClassFromSubpropertyType(subpropertyType: SubPropertyValueType): Vector[OWLAxiom] = {
