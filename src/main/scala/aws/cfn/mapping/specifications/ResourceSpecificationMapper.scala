...
(json: Json, resourceSpecificationName: String)
{
        ...
        case _ =>
          if (isSubprop)
            ...
            JsonUtils.getPrimitiveType(pair._2) match {
              ...
              case Some(CFnType.Integer)    => IntProperty(propName,domain,isReq(pair._2))
              case Some(CFnType.Boolean)    => BooleanProperty(propName,domain,isReq(pair._2))
  ...
  = pair =>
  {
    ...
    if (JsonUtils.isList(pair._2) && JsonUtils.ofPrimitive(pair._2))
      ...
      case Some(CFnType.Integer)        => IntAttribute(attrName,domain)
      case Some(CFnType.Boolean)        => BooleanAttribute(attrName,domain)
