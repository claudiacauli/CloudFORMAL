...
object SpecificationRenaming {
  ...
  val subpropertyTypeName : String => String
    = subPropTypeName =>
    if (subPropTypeName.contains('.')) subPropTypeName.split('.')(1).toLowerCase
    else subPropTypeName.toLowerCase

  val propertyName: (String,String) => String
    = (domainTypeName, propName) => domainTypeName.toLowerCase() + "_" + propName.toLowerCase

  val attributeName: (String,String) => String
    = (domainTypeName, attrName) => "attribute_" + propertyName(domainTypeName,attrName)

  val unambiguousPropertyName: (String,String,String) => String
  = (resTypeName, domainName, propName) =>
